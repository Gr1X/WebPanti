...
<div class="m-10 space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <div class="bg-custom-50 p-4 rounded-2xl shadow">
            <div class="flex justify-between">
                <h3 class="text-md font-semibold text-custom-200">User Account</h3>
                <ion-icon name="ellipsis-horizontal" class="text-white size-5 self-center cursor-pointer" 
        ...
        <div class="bg-custom-50 p-6 rounded-2xl shadow md:w-3/5">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h1 class="text-3xl font-bold text-white">Rp. {{ number_format($totalDonasi, 0, ',', '.') }}</h1>
                </div>
            </div>
        ...
        <div class="bg-custom-50 mt-6 md:mt-0 p-6 rounded-2xl shadow md:w-2/5">
            <div class="mb-4">
                <div class="flex justify-between w-full">
                    <div class="">
                        <h1 class="text-2xl font-bold text-white self-center">Transaksi Terbaru</h1>
                    </div>
                    <a
                        href="{{ route("admin.payments.index") }}"
                        class="uppercase text-sm font-semibold inline-flex items-center rounded-2xl text-custom-200 hover:text-custom-300  hover:bg-green-900 focus:ring-green-900 border-green-900 px-3 py-2">
                        See All
            ...
            <div class="m-2 pb-2 border-b last:border-b-0 border-custom-375">
                <span class="text-gray-300 text-sm font-medium">{{ $donasiItem->waktu_donasi->format('d M Y, H:i') }}</span>
                <div class="text-white text-2xl font-bold">
                    Rp {{ number_format($donasiItem->jumlah, 2, ',', '.') }}
                </div>
                <div class="text-yellow-300 text-sm italic truncate font-medium">{{ $donasiItem->notes }}</div>
            </div>
            @empty
    ...
    <div class="relative mx-10">
        <div class="bg-custom-50 rounded-2xl shadow p-6 h-[500px]">
            <!-- Header -->
            <div class="mb-6 flex justify-between">
                <div class="grid">
                    <span class="text-lg text-custom-200 font-semibold">Table</span>
                    <span id="tableTitle" class="text-white text-4xl font-bold">Program</span>
                </div>
                <div class="flex space-x-2">
                    <button onclick="previousTable()" class="bg-custom-75 hover:bg-custom-100 text-white p-2 rounded-full shadow-md">
                        <ion-icon name="chevron-back-outline" class="w-6 h-6 flex items-center"></ion-icon>
                    </button>
                    <button onclick="nextTable()" class="bg-custom-75 hover:bg-custom-100 text-white p-2 rounded-full shadow-md">
            ...
            <!-- Tabel yang Berganti -->
            <div id="tableContainer" class="relative overflow-x-auto overflow-y-auto max-h-[350px] rounded-lg">
                <!-- Table 1: Program -->
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 program-table">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-custom-400 border-b border-custom-300">
                            <td class="px-6 py-4">
                                @if($program->gambar)
                            ...
                            <td class="px-6 py-4 max-w-2xs truncate font-semibold text-md">
    ...
    const salesChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: rataRata,
                borderColor: '#DFAE48', // Hijau tua
                backgroundColor: 'rgba(255, 238, 218)', // Hijau muda transparan
                pointBackgroundColor: '#FFEDA9', // Oren untuk titik
